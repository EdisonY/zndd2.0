...
export default {
    ...
    methods: {
        ...
        search(value,page){
            ...
            this.$api.get('/api/confType/page',this.searchData).then(res => {
            // this.$api.get('http://172.51.217.202:30800/confType/page',this.searchData).then(res => {
                if(res.success){
                    self.loading = false
        ...
        delet(id){
            var self = this
            this.$api.delete('/api/confType',{id:id}).then(res => {
                if(res.success){
                    self.search()
                    self.$notify.success({
                        title: '成功',
        ...
        sendInfo(){
            ...
            function postApi() {
                if(self.update){
                    self.$api.put('/api/confType',self.sendData).then(res => {
                        if(res.success){
                            self.handleClose()
                            self.search()
                            self.$notify.success({
                    ...
                    self.$api.post('/api/confType',self.sendData).then(res => {
                        if(res.success){
                            self.handleClose()
                            self.search()
                            self.$notify.success({
